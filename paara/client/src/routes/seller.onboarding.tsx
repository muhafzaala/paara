...
import { ArrowRight, ArrowLeft, Check, Loader2, Upload, ShieldCheck, Building2, Camera, CreditCard, Sparkles } from "lucide-react";
...
const STEPS = [
  { id: 0, label: "Shop", icon: Building2, urdu: "دکان" },
  { id: 1, label: "Identity", icon: ShieldCheck, urdu: "شناخت" },
  { id: 2, label: "Craft", icon: Camera, urdu: "ہنر" },
  { id: 3, label: "Payouts", icon: CreditCard, urdu: "ادائیگی" },
];
...
function OnboardingPage() {
  ...
  // Form data — synced from backend profile
  ...
  // Load existing profile (auto-resume)
  ...
  // Hydrate form from backend on first load
  useEffect(() => {
    ...
    // If they've already submitted, send them to status page
  ...
  const submitApplication = async () => {
    ...
    try {
      ...
      await saveMutation.mutateAsync({ bank });
      // Then submit
      ...
      <div className="pt-28 pb-20 px-6 lg:px-12">
        <div className="mx-auto max-w-[1000px]">
          <header className="text-center mb-10">
            <p className="eyebrow inline-flex items-center gap-2"><Sparkles size={12} className="text-[#C9921A]" /> Become a PAARA Artisan</p>
            ...
            <p className="text-sm md:text-base text-[#6B645A] mt-3 max-w-xl mx-auto">
              Four steps. We verify every artisan personally — that's what makes PAARA different.
            </p>
          </header>
          ...
          <div className="mt-10">
            {step === 0 && <ShopStep shop={shop} setShop={setShop} onNext={() => saveAndAdvance({
        ...
        return (
          <div key={s.id} className="flex items-center gap-2 md:gap-3">
            <div className="flex flex-col items-center gap-1">
              <div className="w-12 h-12 rounded-full grid place-items-center transition-all"
                style={{
              ...
              <span className="text-[10px] uppercase tracking-[0.14em] font-bold" style={{ color: reached ? "#1C3A2A" : "#6B645A" }}>
                {s.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className="w-8 md:w-16 h-px" style={{ background: i < step ? "#1C3A2A" : "rgba(28,58,42,0.18)" }} />
            )}
          </div>
        );
      })}
...
function ShopStep({ shop, setShop, onNext }: any) {
  ...
  return (
    <div className="bg-white rounded-[24px] p-6 md:p-10 shadow-[var(--shadow-soft)] border border-[rgba(28,58,42,0.08)] space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="Shop name" value={shop.shopName} onChange={(v: string) => setShop({ ...shop, shopName: v })} placeholder="e.g. Multani Blue Pottery House" />
        <Field label="Year established" value={shop.yearEstablished} onChange={(v: string) => setShop({ ...shop, yearEstablished: v })} placeholder="e.g. 1985" type="number" optional />
      </div>

      <Field label="Short description" value={shop.shopDescription} onChange={(v: string) => setShop({ ...shop, shopDescription: v })} placeholder="One or two lines about your craft (visible on cards)" textarea rows={2} />

      <Field label="Your story" value={shop.shopStory} onChange={(v: string) => setShop({ ...shop, shopStory: v })} placeholder="The longer version — your craft's heritage, your journey, what makes your work special" textarea rows={5} optional />

      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="City" value={shop.city} onChange={(v: string) => setShop({ ...shop, city: v })} placeholder="Multan" />
        <div>
          <label className="block text-xs font-semibold uppercase tracking-[0.16em] text-[#1C3A2A] mb-2">Region</label>
          <select value={shop.region} onChange={(e) => setShop({ ...shop, region: e.target.value })}
            className="w-full bg-[#FFF8EC] border border-[rgba(28,58,42,0.14)] rounded-full px-4 py-3 text-sm focus:outline-none focus:border-[#C9921A]">
...
function IdentityStep({ docs, setDocs, onBack, onNext }: any) {
  ...
  return (
    <div className="bg-white rounded-[24px] p-6 md:p-10 shadow-[var(--shadow-soft)] border border-[rgba(28,58,42,0.08)] space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <UploadTile label="CNIC — Front" value={docs.cnicFront} onChange={(v: string) => setDocs({ ...docs, cnicFront: v })} hint="Clear photo or scan" />
        <UploadTile label="CNIC — Back" value={docs.cnicBack} onChange={(v: string) => setDocs({ ...docs, cnicBack: v })} hint="Clear photo or scan" />
      </div>

      <div className="flex justify-between pt-4">
        <button onClick={onBack} className="text-sm text-[#1C3A2A] hover:text-[#C9921A] inline-flex items-center gap-1">
          <ArrowLeft size={14} /> Back
        </button>
        <button onClick={onNext} disabled={!valid} className="btn btn-primary disabled:opacity-50">
          Continue <ArrowRight size={16} />
        </button>
      </div>
    </div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
        {photos.map((p: string, i: number) => (
          <div key={i} className="relative aspect-square rounded-lg overflow-hidden bg-[#FFF8EC]">
            ...
            <button onClick={() => onChange(photos.filter((_: string, idx: number) => idx !== i))}
...
function BankStep({ bank, setBank, onBack, onSubmit, submitting }: any) {
  const valid = bank.accountHolderName.trim() && bank.accountNumber.trim() && bank.bankName.trim();
  return (
    <div className="bg-white rounded-[24px] p-6 md:p-10 shadow-[var(--shadow-soft)] border border-[rgba(28,58,42,0.08)] space-y-6">
      <div>
        <h2 className="display-serif text-2xl text-[#1C3A2A] mb-1">Where to send your payouts</h2>
        <p className="text-sm text-[#6B645A]">Bank details for your weekly settlement. Encrypted in transit and at rest.</p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="Account holder name" value={bank.accountHolderName} onChange={(v: string) => setBank({ ...bank, accountHolderName: v })} placeholder="As on bank record" />
        <Field label="Bank name" value={bank.bankName} onChange={(v: string) => setBank({ ...bank, bankName: v })} placeholder="HBL / MCB / UBL / Meezan etc." />
      </div>
      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="Account number" value={bank.accountNumber} onChange={(v: string) => setBank({ ...bank, accountNumber: v })} placeholder="" />
        <Field label="IBAN" value={bank.iban} onChange={(v: string) => setBank({ ...bank, iban: v })} placeholder="PKxx xxxx xxxx xxxx xxxx xxxx" optional />
      </div>
      ...
      <div className="bg-[#FFF8EC] rounded-2xl p-4 border border-[rgba(201,146,26,0.2)] flex items-start gap-3">
        ...
        <p className="text-xs text-[#6B645A]">
          Submitting moves your application to <strong className="text-[#1C3A2A]">Documents Under Review</strong>.
          Our verification team will reach out within 3-5 business days.
        </p>
      </div>

      <div className="flex justify-between pt-4">
        <button onClick={onBack} className="text-sm text-[#1C3A2A] hover:text-[#C9921A] inline-flex items-center gap-1" disabled={submitting}>
          <ArrowLeft size={14} /> Back
        </button>
        <button onClick={onSubmit} disabled={!valid || submitting} className="btn btn-primary disabled:opacity-50">
